...
async function researchStory(story) {
  const broadQuery = normalizeTitle(story.seed.title)
    .split(" ")
    .filter((word) => word.length > 3 && !["todo", "tenes", "saber", "duelo", "partido", "fecha", "futbol"].includes(word))
    .slice(0, 8)
    .join(" ");
  const searches = await Promise.all([
    rssSearch(`"${story.seed.title}"`),
    rssSearch(broadQuery || story.seed.title),
  ]);
  const combined = [...story.sources, ...searches.flat()]
...
function buildFreeResearchDraft(story) {
  const distinctSources = story.sources
    .filter((source, index, all) => all.findIndex((candidate) => candidate.source.toLowerCase() === source.source.toLowerCase()) === index)
    .slice(0, 6);
  const evidence = distinctSources.map((source) => {
    const detail = source.description || source.title;
    return `${source.source} informó: ${detail}`;
  });
  const slug = normalizeTitle(story.seed.title).replace(/\s+/g, "-").slice(0, 90);
  return {
    titulo: story.seed.title,
    bajada: `Ficha de investigación automática basada en ${distinctSources.length} fuentes. Requiere revisión y redacción editorial antes de publicar.`,
    cuerpo: [
      "BORRADOR DE INVESTIGACIÓN — NO PUBLICAR SIN REVISAR",
      "",
      "Hecho detectado",
      story.seed.description || story.seed.title,
      "",
      "Información encontrada en otros medios",
      ...evidence.map((item) => `• ${item}`),
      "",
      "Próximo paso editorial",
      "Contrastar los datos, descartar versiones no confirmadas y convertir esta ficha en una nota original de DataSao. Cada enlace consultado queda disponible en el panel.",
    ].join("\n"),
    categoria: "Fútbol argentino",
    tags: normalizeTitle(story.seed.title).split(" ").filter((word) => word.length > 5).slice(0, 5),
    slug,
    seo_descripcion: story.seed.description?.slice(0, 155) || story.seed.title.slice(0, 155),
    nivel_confianza: distinctSources.length >= 3 ? "alto" : "medio",
    alertas: ["Modo gratuito: ficha sin redacción de IA", "Revisión humana obligatoria antes de publicar"],
    hechos_confirmados: evidence,
  };
}
...
export async function runPipeline({ limit = 1 } = {}) {
  ...
  const stories = cluster(candidates).slice(0, 20);
  const results = [];
  for (const story of stories) {
    if (results.some((result) => result.inserted) || results.length >= Math.min(limit, 3)) break;
    try {
      const researched = await researchStory(story);
      const distinctMedia = new Set(researched.sources.map((source) => source.source.toLowerCase()));
      if (distinctMedia.size < 2) throw new Error("No se encontraron al menos dos medios distintos para contrastar la noticia");
      const article = process.env.NEWS_AI_ENABLED === "true"
        ? await generateArticle(researched)
        : buildFreeResearchDraft(researched);
      ...
      if (!String(error.message || error).includes("dos medios distintos")) {
